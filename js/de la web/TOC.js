  ...
  var _TOCNode = declare([_Widget, _Templated], {
    ...
    postCreate: function(){
      ...
      if (!this._noCheckNode) {
        ...
        if(this.data.visible && typeof this.data.id === "number") {
          console.log("Capa default", this.rootLayer.url, this.data.id)
          var capa = {
            nombre: this.data.name,
            id: this.data.id,
            url: this.rootLayer.url,
            serviceId: this.rootLayer.id
          }
          capasDefault.push(capa);
        }
    ...
    _createServiceLayerNode: function(serviceLayer){
      ...
      if (serviceLayer._subLayerInfos && serviceLayer._subLayerInfos.length>0) {// group layer
        ...
        this._noCheckNode = true;/*Eliminar checkbox en grupos*/
        ...
        // Url de FeatureLayer
        this.labelNode.innerHTML += "<br/><span id='" + this.rootLayer.url + "/" + serviceLayer.id + "' class='layerUrl zoomLayer'><img src='imagenes/lupa.png' class='icon'/></span>";
        // Linea de descarga (Solo servicios de ArcGIS server)
        this.labelNode.innerHTML += "&nbsp;&nbsp;<a target='_blank' href='" + this.rootLayer.url + "/generatekml?layerOptions=nonComposite&docName=" + getCleanedString(serviceLayer.name) + "&layers=" + serviceLayer.id  + "' class='layerUrl'><img src='imagenes/descarga.png' class='icon'/></a>";
        // Cluster para las capas
        this.labelNode.innerHTML += "&nbsp;&nbsp;<span data-layerUrl='" + this.rootLayer.url + "/" + serviceLayer.id + "' class='icon-Cluster action active' title='Cluster'></span>";
        // Análisis básico de la capa
        // this.labelNode.innerHTML += "&nbsp;&nbsp;<span id='" + this.rootLayer.id + '_layer' + serviceLayer.id + "_analyze' class='layerUrl' onclick=\"getFeaturesFromLayer('"+ this.rootLayer.url + "/" + serviceLayer.id +"')\"><img src='imagenes/Analisis/analyze.png' class='icon'/></span>";
        // Cambiar Opacidad de la capa
        this.labelNode.innerHTML += '&nbsp;&nbsp;<input id="'+ this.rootLayer.id + '_layer' + serviceLayer.id +'_range" type="range" min="0" max="100" step="1" value="100">';
      ...
      if (t == this.checkNode || dijit.getEnclosingWidget(t) == this.checkNode) {
        ...
        if (this.serviceLayer) {
          ...
          if (this.serviceLayer.visible) {
            ...
            console.log("Habilitado: " + this.serviceLayer.name + " id: " + this.serviceLayer.id );
            var capa = {
              nombre: this.serviceLayer.name,
              id: this.serviceLayer.id,
              url: this.rootLayer.url,
              serviceId: this.rootLayer.id
            }
            ...
            console.log("Deshabilitado: " + this.serviceLayer.id);
            var disableId = this.serviceLayer.id;
            console.log(this);
            var capa = {
              nombre: this.serviceLayer.name,
              id: this.serviceLayer.id,
            ...
            var evt = new CustomEvent('capa-borrada', { 'detail': capa });
            document.dispatchEvent(evt);
          }
    ...
    _getVisibleLayers: function(){ /*quitar checkbox*/
      var vis = [];
      var rootName = this.rootLayer.id;
